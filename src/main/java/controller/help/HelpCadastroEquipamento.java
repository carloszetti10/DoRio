...
public class HelpCadastroEquipamento {
    ...
    for (Retirada r : listarRetiradas) {
        if (Objects.equals(r.getIdEquipamento(), id)) {
            quant += r.getQuantidadeEquipamento();
        }
    }

    System.err.print("quantidade: " + quant);  // Imprime no fluxo de erro para depuração.

    if (quant > 0) {
        return new Retirada(0, 0, quant);
    }
    return null;
    }
    
    /*private Integer pegarQuantAtualizar1(Integer quantBanco, Integer quantPassada, Integer quantDispBanco, Integer id, Boolean tipo ) throws SQLException{
